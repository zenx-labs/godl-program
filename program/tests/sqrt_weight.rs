...
const GODL: u64 = ONE_GODL; // 1e11 grams
const SQRT20: u128 = 4_472_135_954; // effective_multiplier(20 * SCALE)
...
fn pod_account<T: Discriminator + Pod>(value: &T) -> Account {
...
impl EnvBuilder {
    ...
    fn stake(mut self, spec: StakeSpec) -> Self {
    ...
    fn stake_at(mut self, address: Pubkey, spec: StakeSpec) -> Self {
        self.stakes.push((address, spec));
        self
    }

    fn rewards_factor(mut self, f: Numeric) -> Self {
        self.stake_rewards_factor = f;
        self
    }

    fn fund(mut self, k: Pubkey) -> Self {
        self.funded.push(k);
        self
    }

    fn token(mut self, owner: Pubkey, amount: u64) -> Self {
        self.token_accounts.push((owner, amount));
        self
    }

    fn account(mut self, addr: Pubkey, acc: Account) -> Self {
...
async fn get<T: AccountDeserialize + Copy>(ctx: &mut ProgramTestContext, addr: Pubkey) -> T {
    let acc = ctx
        .banks_client
        .get_account(addr)
        .await
...
async fn token_balance(ctx: &mut ProgramTestContext, addr: Pubkey) -> u64 {
...
async fn assert_invariant(ctx: &mut ProgramTestContext, stake_addrs: &[Pubkey]) {
...
fn ix_claim_yield_v2(signer: Pubkey, id: u64, amount: u64, treasury_writable: bool) -> Instruction {
    let stake = stake_v2_pda(signer, id).0;
...
fn spec(authority: Pubkey, id: u64, balance: u64, multiplier: u64, nft: bool, version: u8) -> StakeSpec {
    StakeSpec {
        authority,
        id,
        balance,
