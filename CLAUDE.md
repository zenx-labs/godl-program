...
# Same tests through the native in-process processor (no cargo-build-sbf needed)
cargo test -p godl-program
...
### Gold rewards flow (SOL motherlode → XAUt0)

The 1% `SOL_MOTHERLODE_BPS` skim per round still accumulates in the `sol_motherlode` PDA, but it no longer pays out on a hit. `PreBuyGold` → `BuyGold` (bury-authority gated, same split and raw-Jupiter-data pattern as `PreBury` → `Bury`, gold vault PDA as taker) moves SOL from the motherlode into the `GoldVault`'s WSOL ATA, swaps it to XAUt0 (`XAUT_MINT`, legacy SPL, 6 decimals), and credits `GoldVault::xaut_rewards_factor` split across `treasury.total_unclaimed`. The split is mandatory: a direct lamport debit of a program-owned PDA cannot share an instruction with a CPI unless the debited account is passed to that CPI, and the live token program rejects extra accounts on `SyncNative`. `BuyGold` asserts the vault WSOL is fully consumed, so the route must be quoted for the full balance including dust (the CLI does this). `DepositGold` is the manual-input twin. Each miner's gold state lives in a `MinerExtended` PDA (`miner_extended_pda(authority)`); every handler that touches `rewards_godl` calls `sync_gold_rewards` (program `gold` module) right after `Miner::update_rewards` and before the balance changes, and `None` for the extended account is only legal when the miner holds no unrefined GODL. `DeployWithMinerExtended` creates the extended account next to the miner; `CheckpointWithMinerExtended` never creates it and fails with `MinerExtendedMissing` when it is absent but there is GODL to settle or credit (clients run `CreateMinerExtended` first). Claim GODL, inject and the OTC bonus leg create it lazily with the signer paying rent. Legacy `Deploy` and `Checkpoint` keep their old layouts for un-migrated clients but self-retire at `LEGACY_INSTRUCTION_EXPIRY_TS` (checked against `clock.unix_timestamp`), and `PreBuyGold` / `BuyGold` / `DepositGold` refuse to run before that time (`GoldDistributionLocked`), so a balance can never change without settling and then over-claim. Set the constant to the upgrade time + `6 * ONE_HOUR` right before the release build. `ClaimGold` pays out. The factor is denominated in XAUt0 base units per gram × `GOLD_FACTOR_SCALE`. Rollout order: set `LEGACY_INSTRUCTION_EXPIRY_TS` → upgrade → migrate bots/frontend to the `*WithMinerExtended` instructions within six hours → `initialize-gold-vault` → `create-miner-extended-all` → `audit-unclaimed` (`rebase-total-unclaimed` if drifted) → first `buy-gold` once the expiry time has passed. Tests: `program/tests/gold.rs`.
