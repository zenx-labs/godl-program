...
The 1% `SOL_MOTHERLODE_BPS` skim per round still accumulates in the `sol_motherlode` PDA, but it no longer pays out on a hit. `BuyGold` (bury-authority gated, same raw-Jupiter-data pattern as `Bury`, gold vault PDA as taker) moves SOL from the motherlode into the `GoldVault`'s WSOL ATA, swaps it to XAUt0 (`XAUT_MINT`, legacy SPL, 6 decimals), and credits `GoldVault::xaut_rewards_factor` split across `treasury.total_unclaimed`. `DepositGold` is the manual-input twin. Each miner's gold state lives in a `MinerExtended` PDA (`miner_extended_pda(authority)`). Every handler that touches `rewards_godl` calls `sync_gold_rewards` (program `gold` module) right after `Miner::update_rewards` and before the balance changes; `None` for the extended account is only legal when the miner holds no unrefined GODL. Claim GODL, inject and the OTC bonus leg take the vault + extended accounts and lazily create the extended account with the signer as rent payer. Checkpoint exists twice: the legacy `Checkpoint` keeps its 8-account layout for un-migrated clients and does **not** settle gold, and `CheckpointWithMinerExtended` (same accounts + vault + extended PDA) does. The legacy one self-retires at `LEGACY_CHECKPOINT_EXPIRY_TS` (`LegacyCheckpointExpired`, checked against `clock.unix_timestamp`), and `BuyGold` / `DepositGold` refuse to run before that time (`GoldDistributionLocked`), so a miner can never change its balance without settling and then over-claim. Set the constant to the upgrade time + `6 * ONE_HOUR` right before the release build. `ClaimGold` pays out. The factor is denominated in XAUt0 base units per gram × `GOLD_FACTOR_SCALE`. Rollout order: set `LEGACY_CHECKPOINT_EXPIRY_TS` → upgrade → migrate bots/frontend to `CheckpointWithMinerExtended` within six hours → `initialize-gold-vault` → `create-miner-extended-all` → `audit-unclaimed` (`rebase-total-unclaimed` if drifted) → first `buy-gold` once the expiry time has passed. Tests: `program/tests/gold.rs`.
